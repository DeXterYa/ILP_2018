...
public class ShilActivity extends AppCompatActivity implements ActionMode.Callback {
                ...
                if (queryDocumentSnapshots != null) {
                    ...
                    for (QueryDocumentSnapshot d : queryDocumentSnapshots) {
                        ...
                        if ((markersonmap2.getIsCollected_1() == 1)&&(markersonmap2.getIsStored() == 0)&& (markersonmap2.getIsInMarket() == 0)) {
                            if(markersonmap2.getCurrency().equals("SHIL")) {
                                count += 1;
        ...
        switch (item.getItemId()) {
            case R.id.action_store:
                if ((numberInBank < 25) && ((25-numberInBank)>=selectedIds.size())) {
                    for (Coin coin_shil : mcoin) {
                        if (selectedIds.contains(coin_shil.getId())) {
                            ...
                            DocumentReference documentReference2 = FirebaseFirestore.getInstance()
                                    .collection("Icons").document(firebaseUser.getUid())
                                    .collection("features").document(coin_shil.getTitle());
                    ...
                    Intent intent3 = new Intent(ShilActivity.this, Activity_Two.class);
                    startActivity(intent3);
                    finish();
                    return true;
                } else {
                    Toast.makeText(ShilActivity.this, "You cannot store those coins in bank",
